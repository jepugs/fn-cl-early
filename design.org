...
#+BEGIN_SRC common-lisp
  ...
  (make-parser
    ((program -> (* expr) @eof)
     (expr -> constant / compound / unary / dollar / dot)

     (constant -> @string / @number)

     (compound -> paren / bracket / brace)
     (paren -> @left-paren (* expr) @right-paren)
     (paren -> @left-bracket (* expr) @right-bracket)
  ...
  (defun fn-parse (tokens)
    (funcall
     (make-parser
       ;; syntax is this: (make-parser (PATTERN-NAME PATTERN CALLBACK)* )

       ;; callback is a function of one argument that is called with a list of tokens/patterns that
       ;; match the pattern

       ;; the program pattern returns a list of expressions
       (program
        ((* expr) 'eof)
        #'car)
       ;; generic expression pattern. Doesn't actually correspond to a single type
       (expr
        (constant / compound / unary / dot)
        #'car)
       ...
       (paren
        ...
        $(make-ast-paren :contents (take (- (length $) 2) (cdr $))))
       (bracket
        ('left-bracket (* expr) 'right-bracket)
        $(make-ast-bracket :contents (take (- (length $) 2) (cdr $))))
       (brace
        ('left-brace (* expr) 'right-brace)
        $(make-ast-brace :contents (take (- (length $) 2) (cdr $))))

       ;; unary operators
       (unary 
        ((or quote quasiquote unquote unquote-splicing dollar))
        #'car)
       (quote
        ('quote expr)
        $(make-ast-quote :expr (cadr $)))
       (quasiquote
        ('backtick expr)
        $(make-ast-quasiquote :expr (cadr $)))
       (unquote
        ('comma expr)
        $(make-ast-unquote :expr (cadr $)))
       (unquote-splicing
        ('comma-splice expr)
        $(make-ast-unquote-splicing :expr (cadr $)))

       ;; dollar sign syntax
       (dollar 
        (('dollar-paren (* expr) 'right-paren)
         / ('dollar-bracket (* expr) 'right-bracket)
         / ('dollar-brace (* expr) 'right-brace)
         / ('dollar-backtick expr))
        (lambda (toks)
          (case (token-kind (car toks))
            ((dollar-backtick) 
             (make-ast-dollar :expr (make-ast-quasiquote :expr (cadr toks))))
            ((dollar-paren)
             (make-ast-dollar 
              :expr (make-ast-paren :contents (take (- (length toks) 2) (cdr toks)))))
            ((dollar-bracket)
             (make-ast-dollar 
              :expr (make-ast-bracket :contents (take (- (length toks) 2) (cdr toks)))))
            ((dollar-brace)
             (make-ast-dollar 
              :expr (make-ast-brace :contents (take (- (length toks) 2) (cdr toks))))))))

       ;; dot
       (dot
        ((dot / 'symbol) 'dot 'symbol)
        $(make-ast-dot :left (car $) :right (cadr $))))))
